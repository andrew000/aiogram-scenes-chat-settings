...
from bot.scenes.chat_settings.stages.general_settings.set_farewell.keyboards import (
    farewell_text_keyboard,
)
from bot.storages.psql import DBChatSettingsModel, RDChatSettingsModel

CHAT_SETTINGS_FAREWELL_SET_TEXT_WINDOW_TEXT = (
    "💁‍♂️ In this window, you need to send a message of farewell to the chat members.\n"
    "\n"
...
class SetFarewellTextWindow(BaseScene, state=ChatSettingsStates.FAREWELL_SET_TEXT):
    @on.callback_query.enter()
    async def on_enter_cb(self, cb: CallbackQuery, state: FSMContext) -> None:
        sent = await cb.message.edit_text(
            text=CHAT_SETTINGS_FAREWELL_SET_TEXT_WINDOW_TEXT,
            reply_markup=farewell_text_keyboard(),
        )
    ...
    async def set_farewell_text_handler(
        self,
        msg: Message,
        state: FSMContext,
        ...
        async with db_session() as session:
            ...
            chat_settings.farewell_text = msg.html_text
    ...
    @on.message(F.text & F.text.len() > MAX_GREETING_FAREWELL_LENGTH)
    async def too_long_farewell_text_handler(
        self,
        msg: Message,
    ) -> None:
        await msg.answer(
...
async def reset_farewell_text(
    ...
    async with db_session() as session:
        ...
        if chat_settings.farewell_text is not None:
            updated = True
