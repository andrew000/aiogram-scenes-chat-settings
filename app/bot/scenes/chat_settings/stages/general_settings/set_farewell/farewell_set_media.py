...
CHAT_SETTINGS_FAREWELL_SET_MEDIA_WINDOW_TEXT = (
    "💁‍♂️ In this window, you need to send a media file that the bot will send to members who "
    "have left the chat.\n"
    "\n"
    "📝 A media file can be an image, video, GIF, or sticker.\n"
    "\n"
    "💡 Pay attention:\n"
    "— Depending on the type of media sent, the bot will change the type of farewell "
    "accordingly.\n"
...
class SetFarewellMediaWindow(BaseScene, state=ChatSettingsStates.FAREWELL_SET_MEDIA):
    ...
    async def set_farewell_photo_handler(
        ...
        async with db_session() as session:
            ...
            chat_settings.farewell_photo_id = msg.photo[-1].file_id
    ...
    @on.message(F.video)
    async def set_farewell_video_handler(
        self,
        msg: Message,
        state: FSMContext,
        db_session: async_sessionmaker[AsyncSession],
        ...
        async with db_session() as session:
            ...
            chat_settings.farewell_video_id = msg.video.file_id
            chat_settings.farewell_type = GreetingFarewellType.VIDEO.value
    ...
    async def set_farewell_gif_handler(
        self,
        msg: Message,
        state: FSMContext,
        db_session: async_sessionmaker[AsyncSession],
        ...
        async with db_session() as session:
            ...
            chat_settings.farewell_gif_id = msg.animation.file_id
            chat_settings.farewell_type = GreetingFarewellType.GIF.value
    ...
    async def set_farewell_sticker_handler(
        self,
        msg: Message,
        state: FSMContext,
        db_session: async_sessionmaker[AsyncSession],
        ...
        async with db_session() as session:
            ...
            chat_settings.farewell_sticker_id = msg.sticker.file_id
            chat_settings.farewell_type = GreetingFarewellType.STICKER.value
...
async def reset_farewell_media(
