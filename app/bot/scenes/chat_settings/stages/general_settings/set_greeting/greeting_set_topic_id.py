...
class SetGreetingTopicIDWindow(BaseScene, state=ChatSettingsStates.GREETING_SET_TOPIC_ID):
    ...
    async def on_enter_cb(
        ...
        sent = await cb.message.edit_text(
            CHAT_SETTINGS_SET_TOPIC_ID_WINDOW_TEXT.format(
                stored_topic_id=(chat_settings.greeting_topic_id or "General"),
            ),
            reply_markup=greeting_topic_id_keyboard(),
        )

        await state.update_data(FSMData(current_message_id=sent.message_id))

    @on.message(F.text)
    async def set_greeting_topic_id_handler(
        self,
        msg: Message,
        bot: Bot,
        state: FSMContext,
        ...
        async with db_session() as session:
            ...
            chat_settings: DBChatSettingsModel = await session.scalar(stmt)
            chat_settings.greeting_topic_id = topic_id
